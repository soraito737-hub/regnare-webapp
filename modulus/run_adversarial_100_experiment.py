"""
アドバーサリアル(誤判定を狙った)100件でのLLM判定 実験スクリプト
====================================================================
data/test_dataset_adversarial_100.json(皮肉マーカー無しの婉曲表現・
遠回しなプライバシー詮索・キーワードだけ危険な引っかけ問題)で
llm_classifier.py の弱点を洗い出す。
"""
...
def main(limit: int | None = None):
    ...
    print(f"{len(test_data)}件のアドバーサリアルコメントを読み込みました")

    out_path = Path(__file__).parent.parent / "data" / "experiment_adversarial_100_results.json"
    ...
    for i, item in enumerate(test_data, 1):
        ...
        print(f"  {i}/{len(test_data)}件完了: {item['comment'][:24]} -> {result.judgement}/{result.category}")
        if i < len(test_data):
